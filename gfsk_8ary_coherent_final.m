...
% 8-ary GFSK coherent demodulation simulation — final archive version
...
Fp = 4.5e3;   Fs_stop = 5.5e3;  % Passband/stopband (Hz)
...
freq_no = [-7; -5; -3; -1; 1; 3; 5; 7];  % 8toneIndex

% 8 tones' actual frequencies (Hz), for mixing
...
% ========================================================================
    function [s, sym_gray, bits, Ns] = generate_gfsk(sym_seq, Ns_total)
        ...
        bits(1:3:end) = bitget(sym_seq, 3);  % MSB
        bits(2:3:end) = bitget(sym_seq, 2);  % Middle
        bits(3:3:end) = bitget(sym_seq, 1);  % LSB
        Ns = Ns_in;
    end
...
% ========================================================================
    function [det_sym, det_gray, branch_metric] = detect_coherent(r, Nsym_in, sample_idx_in)
        ...
        for m = 1:M
            ...
            % LowpassFiltering（IsolateOther7tone）
        ...
        det_gray = (det_gray(:) - 1);  % 0..7, ColumnVector Nsym x 1
...
if RUN_FLOOR_ANALYSIS
    ...
    for i = 1:Nsym
        nat = sym_tx_valid(i);
        bits_tx_valid(3*i-2) = bitget(nat, 3);
        bits_tx_valid(3*i-1) = bitget(nat, 2);
        bits_tx_valid(3*i)   = bitget(nat, 1);
    end
    bits_det_valid = zeros(Nsym*k, 1);
    for i = 1:Nsym
        nat = det_sym(i);
        bits_det_valid(3*i-2) = bitget(nat, 3);
        bits_det_valid(3*i-1) = bitget(nat, 2);
        bits_det_valid(3*i)   = bitget(nat, 1);
    end
    ...
    for bt_idx = 1:length(BT_scan)
        ...
        bits_err_bt = 0;
        for i = 1:Nsym
            bits_err_bt = bits_err_bt + (bitget(sym_tx_valid(i),3) ~= bitget(det_sym_tmp(i),3)) ...
                + (bitget(sym_tx_valid(i),2) ~= bitget(det_sym_tmp(i),2)) ...
...
if RUN_H_SCAN
    ...
    for h_idx = 1:length(h_scan)
        ...
        bits_err_h = 0;
        for i = 1:Nsym
            bits_err_h = bits_err_h + (bitget(sym_tx_valid(i),3) ~= bitget(det_sym_h(i),3)) ...
                + (bitget(sym_tx_valid(i),2) ~= bitget(det_sym_h(i),2)) ...
...
for idx = 1:length(EbN0_dB)
    ebno = EbN0_lin(idx);
    % Eb = nsps / k = 16/3 (nsps=16, k=3, Unit power)
    N0 = (nsps / k) / ebno;
    ...
    for sim = 1:Nsim
        ...
        % AWGN
        noise = sqrt(noise_var/2) * (randn(Ns, 1) + 1j*randn(Ns, 1));
        r = s + noise;
        ...
        for i = 1:Nsym
            bit_err_total = bit_err_total + ...
                (bitget(sym_tx_valid(i), 3) ~= bitget(det_sym(i), 3)) + ...
...
% Union bound upper bound (high SNR approximation, for M=8 close to exact theory)
...
semilogy(EbN0_dB, BER_bound, 'm:', 'LineWidth', 1.5, 'DisplayName', 'Union Bound BER');
...
title(sprintf('8-ary GFSK Coherent Detection (h=%.1f, BT=%.1f, nsps=%d)', h, BT, nsps));
...
subplot(3,2,2);
[Hch, fch] = freqz(ch_coeffs, 1, 4096, Fs);
plot(fch/1e3, 20*log10(abs(Hch)));
hold on;
for m = 1:M
    xline(tone_freq(m)/1e3, 'r--', sprintf('f_%d', m-1));
end
xlabel('Frequency (kHz)'); ylabel('Magnitude (dB)');
title('Channel Filter Response & Tone Frequencies');
grid on; xlim([-6 6]); ylim([-100 5]);

% 3.3 8-branch output (time domain)
subplot(3,2,3);
...
legend('B0', 'B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'Decision', 'Location', 'best');
