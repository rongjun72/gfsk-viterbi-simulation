...
> **为什么防护带用 symbol 0？** 实际上防护带选择 symbol 0、1、2 或 3 对参考模板结果几乎无影响。原因有二：① `N_guard = 12` 远大于高斯滤波器 `span = 4` 的脉冲响应长度，防护带→prev 的频率过渡在到达 prev 时早已完全结束，prev 和 curr 的波形完全由自身决定；② tone-mixer 输出取 **模值** `abs(y_lpf)`，累积相位差异被完全消除。选择 symbol 0 仅出于代码便利（`zeros` 数组）。**唯一约束：所有 16 个参考模板必须使用完全相同的防护带**，确保模板间可比性。
