...
% 4-ary GFSK 相干解调仿真 含 仿真报告版
% 功能：tone-mixer 软检测、理论BER对比、无SNR下误码率
%
% 关键经验教训记录：
%   - total_delay 必须包含 gauss_filt 的群延迟（约16个采样点），
%     否则解调时系统总偏移1个符号，导致BER≈0.5。
%   - 使用动态 N_pre/N_post = ceil(total_delay/nsps)+5，确保滤波器完全稳态。
%   - tone LPF fc = 0.75 * 相邻tone间距，以平衡邻道干扰抑制与符号内ISI。
...
BT      = 0.5;          % 高斯滤波BT
span    = 4;            % 高斯滤波span（符号数）
Nsym    = 10000;        % 有效符号数（含前导/后导）

EbN0_dB = 12*log10(1:1.9:20)/log10(20);  % 对数EbN0分布0~12dB，低SNR密集
Nsim    = 1;            % 每EbN0点Monte Carlo次数，1次足够平滑

RUN_FLOOR_ANALYSIS = true;   % 开启无噪声误码率 + 不同BT对比
RUN_H_SCAN         = true;   % 不同调制指数h对比
...
% 1.3 Tone低通滤波器（Chebyshev窗36阶，fc=0.75*tone间距）
tone_spacing = h * Rs;
